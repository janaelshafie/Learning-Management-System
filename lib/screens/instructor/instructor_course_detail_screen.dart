...
class _InstructorCourseDetailScreenState
      ...
      if (response['status'] == 'success') {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(response['message'] ?? 'Failed to load course')),
          );
        }
      }
    } catch (e) {
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              runSpacing: 12,
              children: [
                _buildSummaryChip('Credits', info['credits']?.toString() ?? '0'),
                _buildSummaryChip(
                  'Department',
  ...
  Widget _buildSectionCard(Map<String, dynamic> section) {
    final List<Map<String, dynamic>> students =
        List<Map<String, dynamic>>.from(section['students'] ?? []);
    return Card(
  ...
  void _openGradeEditor(Map<String, dynamic> student) {
    ...
    final Map<String, TextEditingController> controllers = {
      'midterm':
          TextEditingController(text: _formatGradeValue(grade['midterm'])),
      'project':
          TextEditingController(text: _formatGradeValue(grade['project'])),
      ...
      'attendance':
          TextEditingController(text: _formatGradeValue(grade['attendance'])),
  ...
  Future<void> _submitGradeUpdate(
    ...
    final response =
        await _apiService.updateStudentGrade(enrollmentId, payload);
    if (mounted) {
      if (response['status'] == 'success') {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Grades updated')),
        );
