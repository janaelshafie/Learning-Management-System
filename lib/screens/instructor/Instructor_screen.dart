...
class _InstructorScreenState extends State<InstructorScreen>
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (_isProfessor) _buildNavItem(4, 'Registration Requests', Icons.pending_actions),
  ...
  Widget _buildMainContent() {
    switch (_selectedIndex) {
      ...
      case 4:
        return _isProfessor ? _buildRegistrationRequests() : _buildInstructorDashboard();
      case 5:
                ...
                Expanded(
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 24),
                      _buildMessagesCard(_announcements
  ...
  Widget _buildScheduleRow(Map<String, dynamic> course) {
    final String code = course['courseCode']?.toString() ??
                    ...
                    child: Column(
                      ...
                      children: [
                        ListTile(
                          ...
                          subtitle: Text(
                            '$courseCode • $semester • $department',
                          ),
                          trailing: Column(
  ...
  bool get _isProfessor =>
      (_instructorType ?? '').toLowerCase() == 'professor';
  ...
  Future<void> _loadPendingRequests() async {
    ...
    try {
      final response = await _apiService.getPendingRequests(_userData!['userId']);
      if (response['status'] == 'success') {
        final data = response['data'] ?? {};
        setState(() {
          _pendingRegistrations = List<Map<String, dynamic>>.from(data['pendingRegistrations'] ?? []);
          _pendingDrops = List<Map<String, dynamic>>.from(data['pendingDrops'] ?? []);
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(response['message']?.toString() ?? 'Error loading requests')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _handleRequestAction(int enrollmentId, String action) async {
    ...
    try {
      ...
      if (response['status'] == 'success') {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response['message']?.toString() ?? 'Request processed successfully'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response['message']?.toString() ?? 'Error processing request'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget _buildRegistrationRequests() {
    ...
    if (_selectedIndex == 4 && !_isLoadingRequests && _pendingRegistrations.isEmpty && _pendingDrops.isEmpty) {
        ...
        children: [
          ...
          if (_isLoadingRequests)
            ...
            if (_pendingRegistrations.isNotEmpty) ...[
              ...
              const SizedBox(height: 12),
              ..._pendingRegistrations.map((request) => _buildRequestCard(request, 'registration')),
              const SizedBox(height: 24),
            ],
            if (_pendingDrops.isNotEmpty) ...[
              ...
              const SizedBox(height: 12),
              ..._pendingDrops.map((request) => _buildRequestCard(request, 'drop')),
            ],
          ],
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(
                  requestType == 'registration' ? Icons.add_circle : Icons.remove_circle,
                  color: requestType == 'registration' ? Colors.green : Colors.red,
                ),
            ...
            Row(
              ...
              children: [
                ...
                const SizedBox(width: 8),
                ElevatedButton(
                  onPressed: () => _handleRequestAction(enrollmentId, 'approve'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.green,
                    foregroundColor: Colors.white,
                  ),
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => InstructorCourseDetailScreen(
          instructorId: instructorId is int ? instructorId : int.parse(instructorId.toString()),
          course: course,
        ),
      ),
