...
public interface RoomReservationRepository extends JpaRepository<RoomReservation, Integer> {
    ...
    List<RoomReservation> findByRelatedDepartmentId(Integer departmentId);
    List<RoomReservation> findByRelatedInstructorId(Integer instructorId);
    List<RoomReservation> findByAssignmentType(String assignmentType);
    ...
    @Query("SELECT r FROM RoomReservation r WHERE r.status = 'approved' " +
           "AND r.startDatetime >= :startDate AND r.startDatetime <= :endDate " +
           "ORDER BY r.startDatetime")
    List<RoomReservation> findApprovedReservationsByDateRange(
        @Param("startDate") Timestamp startDate,
        @Param("endDate") Timestamp endDate
    );
    
    @Query("SELECT r FROM RoomReservation r WHERE r.status IN ('pending', 'approved') " +
           "ORDER BY r.startDatetime")
    List<RoomReservation> findAllActiveReservations();
}
