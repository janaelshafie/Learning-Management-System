...
class _QuizTakingScreenState extends State<QuizTakingScreen> {
  ...
  bool _isQuizFinished = false; // Track if quiz is finished/submitted

  List<dynamic> _questions = [];
  Map<int, dynamic> _answers = {}; // questionId -> answer data
  Map<int, dynamic> _studentAnswers = {}; // questionId -> submitted answer data with grades
  ...
  bool _isPastDueDate() {
    final dueDateStr = widget.quiz['dueDate']?.toString();
  ...
  Future<void> _checkDueDate() async {
    if (_isPastDueDate()) {
      ...
      final quizId = widget.quiz['quizId'] as int;
      ...
      try {
        ...
        // If past due and not submitted, deny access completely
        if (!hasAnswers) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Quiz is past due date. You cannot take it anymore.'),
    ...
    try {
      ...
      if (mounted && response['status'] == 'success') {
        ...
        setState(() {
          ...
          if (answersMap.isNotEmpty) {
            // Show results if all answers are graded OR if quiz was previously finished
            _showResults = allGraded || _isQuizFinished;
          }
        });
      }
    } catch (e) {
      ...
      body: _isLoading
          ...
          : _questions.isEmpty
              ...
              : ListView(
                  ...
                  children: [
                    if (_showResults) ...[
                      // Show total grade at the top when viewing results
                    ...
                    if (!_showResults) ...[
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: _isSubmitting || _isQuizFinished
                              ? null
                              : () async {
                                  setState(() {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('Quiz submitted successfully. You can view results once it is graded.'),
                                      backgroundColor: Colors.green,
                                    ),
                                  );
                          ...
                          child: Text(_isQuizFinished ? 'Quiz Finished' : 'Finish Quiz'),
                        ),
                      ),
                    ],
                  ],
  ...
  bool _isAnswerCorrect(Map<String, dynamic> question, Map<String, dynamic> studentAnswer, String questionType) {
    ...
    // For SHORT_TEXT, we can't auto-determine correctness, rely on grade
