...
class _ParentMessagingScreenState extends State<ParentMessagingScreen>
  ...
  void initState() {
    ...
    if (widget.students.isNotEmpty) {
      _selectedStudentId = widget.students[0]['studentId'];
      _loadRecipients();
    }
  ...
  Future<void> _loadRecipients() async {
    ...
    try {
      final response = await _apiService.getParentRecipients(
        widget.userId,
        _selectedStudentId!,
      );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          else if (_isLoadingRecipients)
            const Center(child: CircularProgressIndicator())
          else if (_recipients.isEmpty)
            const Text('No recipients available for this student')
          else
            DropdownButtonFormField<int>(
